...
export const DeleteRoleById = (roleId: number) => {
	return http.delete<ResultData>(PORT_ADMIN + Api.Delete + '/' + roleId)
}

/** 新增角色 */
export const AddRole = (data: Role.AddRoleRequestData) => {
	return http.post<ResultData>(PORT_ADMIN + Api.AddRole, data)
}

/** 更新角色 */
export const UpdateRole = (data: Role.UpdateRoleRequestData) => {
	return http.put<ResultData>(PORT_ADMIN + Api.UpdateRole, data)
}

/** 获取角色字典 */
export const GetRoleDict = () => {
	return http.get<ResultData>(PORT_ADMIN + Api.GetRoleDict)
}

/** 根据用户 id 获取角色信息 */
export const GetRoleByUserId = (userId: number) => {
	return http.get<ResultData>(PORT_ADMIN + Api.GetRoleByUserId + '/' + userId)
}
