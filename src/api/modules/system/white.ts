...
export const AddWhite = (data: White.AddWhiteRequestData) => {
	return http.post<ResultData>(PORT_ADMIN + Api.AddWhite, data)
}

/** 更新白名单 */
export const UpdateWhite = (data: White.UpdateWhiteRequestData) => {
	return http.put<ResultData>(PORT_ADMIN + Api.UpdateWhite, data)
}

/** 删除白名单 */
export const DeleteWhite = (whiteId: number) => {
	return http.delete<ResultData>(PORT_ADMIN + Api.Delete + '/' + whiteId)
}
